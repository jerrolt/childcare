...
var FingerprintSdkTest = (function () {
    function FingerprintSdkTest() {
        ...
        this.sdk.onQualityReported = function (e) {
            ...
            document.getElementById("qualityInputBox").innerHTML = Fingerprint.QualityCode[(e.quality)];
        }
...
function onLogin(){
	var fp = localStorage.getItem("imageSrc");
	if(currentFormat == Fingerprint.SampleFormat.PngImage){
		fp = localStorage.getItem("imageSrc"); 
	}
	if(currentFormat == Fingerprint.SampleFormat.Intermediate){
		fp = localStorage.getItem("intermediate");
	}
...
function sampleAcquired(s){   
            ...
            {   	            	       
                ...
                if(state == document.getElementById("content-capture")){ 
                    ...
                    var fpdata = {
	                    'guardian_id':3,
	                    'fingerprint_png':localStorage.getItem("imageSrc")
                    };
                    
                    console.log(fpdata);
                    $.ajax({
				        url: '/api/guardian/save-fingerprint',
				        type: 'PUT',
            ...
            {  
                ...
                localStorage.setItem("intermediate", "");
                var samples = JSON.parse(s.samples);
                console.log("Intermediate Format");
                console.log(samples[0]);
                var sampleData = Fingerprint.b64UrlTo64(samples[0].Data);
                localStorage.setItem("intermediate", sampleData);

//console.log("UTF8 Decoded");JSON.parse
var decodedData = JSON.stringify(samples[0]) ;
...
function checkReaderCount(sucessObj,checkForRedirecting){
   if(sucessObj.length == 0){
    alert("No reader detected. Please insert a reader.");
        ...
        document.getElementById("content-capture").style.display = 'block';
