            ...
            for (i=0;i<readersArray.length;i++){ 
                ...
                "<div id='dynamicInfoDivs' align='left'>"+
                    "<div data-toggle='collapse' data-target='#"+readersArray[i]+"'>"+
                        "<img src='/images/scanfp/info.png' alt='Info' height='20' width='20'> &nbsp; &nbsp;"+readersArray[i]+"</div>"+
...
// handles fp authentication 
function onLogin(){
	//alert('onLogin now');
	console.log(localStorage.getItem("imageSrc"));
	$.post('/api/guardian/fingerprint-login',
	{
		fingerprint: localStorage.getItem("imageSrc")
	},
	function(data,status){	                 
       //redirect to the CheckIn page
       console.log("FPLOGIN Guardian: ")
       console.log(data);
       if(data.data.id)
       {
	       ...
	       //btoa();
	       //atob();	       
		   //window.location.href = "/guardian/checkin/" + data.data.id;
		   
		   console.log('/guardian/checkin/' + data.data.id);
		   parent.$("#fpcheckinFrame").attr('src','/guardian/checkin/' + data.data.id);
		   parent.$('#fpcheckinModal').modal('show');
       } else {
	       window.reload();
       }
...
function onImageDownload(guardian_id){
    if(currentFormat == Fingerprint.SampleFormat.PngImage){
	    console.log("onImageDownload: " + guardian_id);
        if(localStorage.getItem("imageSrc") == "" || localStorage.getItem("imageSrc") == null || document.getElementById('imagediv').innerHTML == "" ){
           alert("No image to download");
			...
			formData.append('fingerprint', localStorage.getItem("imageSrc"));
			//headers: {'Content-Type':'multipart/form-data'},
			//fetch('/api/guardian/fingerprint',{method:'POST', body:formData});
			
			
			$.post('/api/guardian/fingerprint',
			{
	        ...
	        function(data,status){
	            ...
	            parent.$('#fpscanModal').modal('hide');
	            //this.$parent.loadGuardians();
	            parent.window.location.reload();
	             //console.log(data);
	            //window.location.href = window.location.href;
	            
	            /*
	            var vm = this.$parent.loadGuardians();
			    $.get('/api/guardians', function(data, status){
				    confirm(vm.guardians[2].firstname);
				    vm.loadGuardians() = data.data;
				    vm.setPagination(data.meta, data.links);
				});
				*/
	            //**** DONT call downloadURI below - no need to download ****/
	        });
...
console.log(localStorage.getItem("raw"));
            let formData = new FormData();
			formData.append('guardian_id', guardian_id);
			formData.append('fingerprint', localStorage.getItem("raw"));
			//headers: {'Content-Type':'multipart/form-data'},
			//fetch('/api/guardian/fingerprint',{method:'POST', body:formData});
			...
			{
	          guardian_id: guardian_id,
	          fingerprint: localStorage.getItem("raw")
	        },
	        function(data,status){
